...
import {
  ...
  parseVisualIdentity,
  resolveBrandCatalog,
  selectConceptForDraft,
} from "../_shared/image-concept-catalog.ts";
...
async function loadContext(client: Client, job: Job) {
  ...
  const brand = await client
    .from("brands")
    .select(
      "id,name,brand_profiles(audience_definition,positioning,content_pillars,restricted_topics,voice_settings,generation_defaults,visual_system,visual_identity)",
    )
    .eq("id", job.brand_id)
    .single();
...
async function imageStage(
  ...
  const visualIdentity = parseVisualIdentity(profile.visual_identity);
  const brandCatalog = resolveBrandCatalog(visualIdentity);
  const primary =
    visualIdentity.palette.primary ??
    String(visual.primaryColor ?? visual.primary_color ?? "#D14B2A");
  const accent =
    visualIdentity.palette.accent ?? String(visual.accentColor ?? visual.accent_color ?? "#D14B2A");
  const source = relation(context.opportunity.source_documents);
  const sourceLabel = (() => {
    try {
  ...
  for (const draft of (drafts.data ?? []).slice(0, maximum)) {
    const version = relation(draft.post_versions);
    const archetype = selectConceptForDraft({
      seed: String(draft.current_version_id ?? draft.id),
      contentStyle: String(draft.content_style),
      catalog: brandCatalog,
      preferredStyle: visualIdentity.preferredStyle,
    });
    const keyFor = (value: { id: string }) =>
      `concept_${value.id.replace(/[^a-z0-9]/g, "").slice(0, 34)}`;
    const conceptKey = keyFor(archetype);
    const prompt = buildImagePrompt({
      systemPrompt: LIGHTWEIGHT_IMAGE_SYSTEM_PROMPT,
    ...
    images.push({
      ...
      conceptDirection: {
        ...
        concepts: [archetype, ...brandCatalog.filter((entry) => entry.id !== archetype.id)]
          .slice(0, 3)
          .map((entry, index) => ({
            conceptKey: keyFor(entry),
            rank: index + 1,
            title: entry.title,
            visualNucleus: entry
              .brief(String(version.hook ?? context.opportunity.value_nucleus ?? ""))
              .slice(0, 1400),
