create table private.api_rate_limit_counters (
  scope text not null check (scope in ('user', 'internal')),
  subject_hash text not null check (subject_hash ~ '^[0-9a-f]{64}$'),
  operation text not null check (operation ~ '^[a-z0-9_:/.-]{1,180}$'),
  window_started_at timestamptz not null,
  window_ends_at timestamptz not null,
  request_count integer not null check (request_count > 0),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  primary key (scope, subject_hash, operation, window_started_at),
...
create index api_rate_limit_counters_expiry_idx
  on private.api_rate_limit_counters (window_ends_at);

revoke all on private.api_rate_limit_counters from public, anon, authenticated;
grant select, insert, update, delete on private.api_rate_limit_counters to service_role;
